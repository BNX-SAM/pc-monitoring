...
📊 대시보드:
   ...
   - Network: http://192.168.2.76:5000
...
- 같은 네트워크의 다른 PC: `http://192.168.2.76:5000` (서버가 표시한 Network IP 사용)

### 3. 클라이언트 배포

#### 🚀 원클릭 설치 (권장)

가장 쉬운 방법: SMB 공유에서 직접 실행

1. 서버 PC에서 `client/` 폴더를 SMB 공유로 설정
2. 각 직원 PC에서 **PowerShell을 관리자 권한으로 실행**
3. 다음 명령어 실행:

```powershell
powershell -ExecutionPolicy Bypass -File "\\서버PC\공유폴더\client\quick-install.ps1"
```

완료! 매일 오후 12시에 자동으로 데이터 수집이 시작됩니다.

#### 📖 다른 설치 방법

- **배치 파일 사용**: `install.bat`를 관리자 권한으로 실행
- **수동 설치**: 파일을 직접 복사하고 설정
- **대량 배포**: GPO 또는 PowerShell 원격 실행
